...
void drawScene(GLFWwindow* window, float angle, int numerobiektu) {
	...
	//glUniform4f(spLambertTextured->u("lp"), 0, 0, jazda, jazda);
		...
		for (int x = 0; x < numerobiektu + 1; x++)
	{
			...
			if (x == 1)
			{
				...
				Mt = glm::translate(Mt, glm::vec3((float)jazda, jazda2, jazda3));
				M = Mt;
				glUniformMatrix4fv(spLambertTextured->u("M"), 1, false, glm::value_ptr(M));
...
int main(void)
{
	...
	przygotuj_obiektVer2("model//pokojzestolikiem.obj", numerobiektu);
